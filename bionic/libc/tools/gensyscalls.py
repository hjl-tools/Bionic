...
# x32 assembler templates for each syscall stub
#

x32_header = """/* autogenerated by gensyscalls.py */
#include <sys/linux-syscalls.h>
...
class State:
    ...
    def x32_genstub(self, fname, numparams, idname):
        t = { "fname"  : fname,
              "idname" : idname }
	...
	if numparams == 6:
	    ...
	    print "too many parameters: %d 6. aborting" % r
	    sys.exit(1)
        result    += "    pushq   %rbx\n"
    ...
    def x32_genstub_cid(self, fname, numparams, idname, cid):
        # We'll ignore numparams here because in reality, if there is a
        # dispatch call (like a socketcall syscall) there are actually
        # only 2 arguments to the syscall and 2 regs we have to save:
        #   %ebx <--- Argument 1 - The call id of the needed vectored
        ...
        for t in self.syscalls:
            ...
            if t["id2"] >= 0:
                if t["cid"] >= 0:
                    ...
                    t["asm-x32"] = self.x32_genstub_cid(syscall_func, len(syscall_params), "__NR_"+syscall_name, t["cid"])
                else:
    ...
    def gen_arch_syscalls_mk(self, arch):
        ...
        arch_test = {
            ...
            "x32": lambda x: x.has_key("asm-x32"),
    ...
    def gen_syscall_stubs(self):
        for sc in self.syscalls:
            ...
            if sc.has_key("asm-x32") and 'x32' in all_archs:
                fname = "arch-x32/syscalls/%s.S" % sc["func"]
                D( ">>> generating "+fname )
                fp = create_file( fname )
                fp.write(sc["asm-x32"])
                fp.close()
